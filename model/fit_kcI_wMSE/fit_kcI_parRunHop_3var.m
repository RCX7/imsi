...
parfor i=1:n_sims
    ...
    if all(~isinf(run_w_star)) && all(~isinf(hop_w_star))
        [~, run_mdl_forces, run_exp_forces] = norm_and_plot_mdl_vs_exp( ...
                                run_w_star, run_average_stride_ftotal, ...
                                k, c, t_sim, run_tstride_avg, ...
                                num_mdl_pts, num_exp_pts, mse_npoints ...
                            );
        [~, hop_mdl_forces, hop_exp_forces] = norm_and_plot_mdl_vs_exp( ...
                            hop_w_star, hop_average_stride_ftotal, ...
                            2*k, 2*c, t_sim, hop_tstride_avg, ...
                            num_mdl_pts, num_exp_pts, mse_npoints ...
                        );
    
        %% measure MSE and store
        mse = fcurve_mse(run_mdl_forces, run_exp_forces) + ...
            fcurve_mse(hop_mdl_forces, hop_exp_forces);
        mse_landscape(i) = mse;
    end
end
